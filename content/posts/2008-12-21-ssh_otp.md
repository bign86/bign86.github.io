...
One Time Passwords (OTP) are throwaway passwords that can be used only once. They were very popular before SSH when login to remote servers happened through unprotected protocols like `rcp`, `rsh`, `rlogin`, `telnet`, exposed to Man-In-The-Middle attacks. SSH has made OTP passwords less common. However, OTPs maybe still be very useful in particular context, e.g. if you need to login to a remote server from a public PC where someone can have installed a keylogger.
...
The creation tool asks for a secret phrase that is used to generate a seed (in the above case "he6074") and a set of 500 passwords. The last line reports the first password of the set, the one to use at the next login. At each successful login, the system provides the next password from the set.
...
OTPs can be used in conjunction with SSH as well if the SSH daemon has been configured to enable them. In the SSH configuration file, on Debian `/etc/pam.d/sshd` (on different distributions may be different), add the following line
