...
If assumption 1 is verified, all investors agree on the returns distribution, and therefore on the expected return and variance, of each asset. All investors will therefore calculate the same efficient frontier and individuate the same tangent portfolio $T$.<br>
...
* Fama and MacBeth (1973)
  * *Approach*: perform the regression month-by-month (on monthly data) than take an average of the resulting series.<br>The standard errors in the regression are determined by the month-to-month variation in the regression coefficients.
